...
derives NPMI as the primary association measure and constructs a NetworkX
graph with Louvain community detection for career track grouping. The NPMI
graph feeds edge weights in the pathway graph, and the PPMI DataFrame feeds
gap ranking in resume matching.
"""
...
class CooccurrenceNetwork:
    ...
    Receives the binary skill matrix from `SkillVectorizer`, computes the
    thresholded co-occurrence matrix eagerly, and exposes NPMI, graph
    construction, and Louvain community detection as cached properties and
    on-demand methods.
    ...
    identifying the point where community quality begins to degrade. The
    floor of 3 protects PMI stability at small corpus sizes where a single
    ...
    def __init__(
        ...
        """
        Compute the thresholded co-occurrence matrix from the binary
        presence/absence matrix produced by `SkillVectorizer.binary_matrix`.

        When `min_cooccurrence` is `"auto"`, the threshold is selected by
        finding the knee of the modularity-vs-threshold curve. Otherwise the
        float fraction is applied to the corpus size with a floor of 3 to
        protect PMI stability.
    ...
    def npmi_matrix(self) -> csr_array:
        ...
        Bounded to [0, 1] by clipping. Pairs where both skills appear in
        every posting use a floored denominator rather than dividing by
        zero.
    ...
    def _build_cooccurrence(
        ...
        """
        Compute document frequencies and the thresholded co-occurrence
        matrix from the binary presence/absence matrix.

        Converts to CSC for efficient column access, derives document
        frequencies from column pointers, multiplies B^T @ B to get pairwise
        co-occurrence counts, zeros the diagonal, and applies a threshold
        selected via modularity knee detection (auto) or a corpus fraction
        with a floor of 3.
    ...
    def _find_threshold(self, C: csr_array) -> int:
        ...
        Sweeps integer thresholds from the floor of 3 up to the point where
        the graph has no edges, computes Louvain modularity at each level,
        and returns the knee of the modularity-vs-threshold curve via
        `kneed.KneeLocator`.
        ...
        community structure. Falls back to the floor of 3 when the sweep
        produces fewer than 3 candidate thresholds or `kneed` cannot locate
        a knee.

        Args:
            C: Pre-computed co-occurrence matrix with diagonal zeroed but
               not yet thresholded.
    ...
    def _pmi_values(self) -> np.ndarray:
        ...
        Returns the flat data array aligned with `self.cooccurrence.data`
        for direct assignment into a structural copy of the cooccurrence
        matrix.
        """
        C    = self.cooccurrence
    ...
    def graph(self) -> nx.Graph:
        ...
        Returns:
            Undirected `nx.Graph` with skill-name nodes and `weight` edge
            attributes.
        """
    ...
    def ppmi_dataframe(self) -> pd.DataFrame:
        ...
        `ResumeMatcher` gap ranking. Computed and materialized in a single
        step because no other consumer needs the sparse PPMI matrix.

        Returns:
            Square `pd.DataFrame` with skill names as both index and
            columns, values clipped to non-negative.
        """
