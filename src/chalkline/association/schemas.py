...
class AprioriResult(BaseModel, extra="forbid"):
    ...
    Jaccard overlap against PPMI positive pairs, and a summary of
    the top association rules with their metrics.
    """
...
class CommunityLabel(BaseModel, extra="forbid"):
    ...
    within the community, serving as the community's descriptor
    for downstream labeling in the pathway graph.
    """
...
class GraphDiagnostics(BaseModel, extra="forbid"):
    ...
    Modularity is None when the graph is degenerate (no edges or
    a single connected component with all nodes in one community).
    """
...
class MeasureComparison(BaseModel, extra="forbid"):
    ...
    Used to compare PPMI, NPMI, and G-test graph structures
    side by side.
    """
