...
O*NET Task+DWA embeddings, BM25 weighting configuration, and the full
match result with reach exploration.
"""
...
class BM25Config(BaseModel, extra="forbid"):
    ...
    The `length_weight` parameter controls how much task description
    length affects scoring (0 = no normalization, 1 = full
    normalization). The `saturation` parameter controls how quickly
    term frequency gains diminish (higher = slower saturation). With
    stem sets where each term appears at most once, `saturation`
    primarily affects the ratio of IDF contribution to length penalty.
    """
    ...
    def base_penalty(self) -> float:
        """
        Length-independent portion of the BM25 denominator,
        `1 - length_weight`. When `length_weight` is 0, the
        denominator collapses to `1 + saturation` regardless of
        document length.
        """
    ...
    def numerator(self) -> float:
        """
        BM25 numerator `saturation + 1`, constant across all terms
        and documents for a given config.
        """
