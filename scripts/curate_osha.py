...
then extracts section `label_description` values as skill terms with
Zipf frequency filtering to exclude generic headings.
...
def is_generic(term: str) -> bool:
    """
    Test whether a single-word section title is too generic for the
    skill lexicon.

    Uses the same Zipf frequency threshold (4.0) applied by the O*NET
    and supplement curation scripts. Multi-word terms are kept
    unconditionally because eCFR phrases like "fall protection" and
    "fire prevention" combine common words into domain-specific
    safety terms that a per-word filter would incorrectly exclude.
...
def walk(node: dict, allowed_subparts: set[str] | None = None):
    ...
    stripped of trailing periods, then filtered via Zipf frequency to
